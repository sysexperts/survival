...
signal reached_furnace(cell: Vector2i, kind: String)
...
signal placed_building(id: String, cell: Vector2i, orient: int, started: float)
...
func _physics_process(delta: float) -> void:
	...
	if input == Vector2.ZERO:
		...
		if _reach_furnace != INVALID_CELL:
			...
			reached_furnace.emit(fc, _reach_furnace_kind)
			_play("idle")
			return
		if _reach_bed:
			_reach_bed = false
...
## Raute, `orient` die Blickrichtung (0..7), `started` der Baubeginn (Unix-Zeit;
## 0.0 = jetzt). Gibt false zurueck, wenn dort kein ebener Platz ist - dann wird
## auch kein Item verbraucht.
func place_building_at(id: String, top: Vector2i, orient := 0, started := 0.0) -> bool:
	...
	var b = Building.create(world, id, top, lvl, orient, started)
	b.cells = cells
	...
	b.tree_exiting.connect(func():
		for c in taken:
			world.unblock_cell(c))
	placed_building.emit(id, top, orient, started)
	return true
...
func walk_to_furnace(clicked: Vector2i) -> bool:
	...
	if node is Campfire:
		fkind = "cook"
		key = node.cell
	elif node is Furniture and String(node.id) == "eritme_firini":
		fkind = "smelt"
		key = clicked
	else:
		return false
	...
	if stand.x == 2147483647:
		return false
	_reach_furnace_kind = fkind
	if stand == here:
		reached_furnace.emit(key, fkind)
		return true
	path = GridPath.find(world, here, stand, max_step)
	if path.is_empty():
		return false
