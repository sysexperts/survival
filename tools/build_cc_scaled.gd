...
## Spielzustand -> (Quellordner, Spaltenzahl[, Quell-Startspalte]). Die "_hold"-
## Sätze tragen die Körperpose mit Werkzeug in der Hand (inkl. Layer13_Axe /
## NegativeLayer1_Axe). run/axe brauchen keinen eigenen _hold-Satz: dieselbe
## Körperpose, und die Axt-Layer liegen bereits in cc_scaled/run bzw. /axe.
##
## `sleep` ist die LIEGE-Pose: das Pack hat keine eigene Schlaf-Animation, aber
## die "Die"-Animation endet flach am Boden liegend. Wir nehmen daraus nur das
## LETZTE Bild (Quell-Startspalte 5 von 6) als einzelnes, ruhendes Liege-Frame -
## deshalb der optionale dritte Wert (Startspalte). Ohne ihn wird ab Spalte 0
## gelesen.
const ACTIONS := {
	"idle":  ["Idle", 8],
	"walk":  ["Walk", 6],
...
func _init() -> void:
	...
	for state_key in ACTIONS:
		...
		# Optionale Quell-Startspalte (Standard 0). So schneiden wir bei "sleep"
		# nur das liegende Endbild aus der laengeren "Die"-Animation heraus.
		var src_start: int = int(ACTIONS[state][2]) if ACTIONS[state].size() > 2 else 0
		...
		while f != "":
			if f.ends_with(".png"):
				_scale_one(src_dir + f, dst_dir + f, cols, src_start)
				total += 1
			f = d.get_next()
		d.list_dir_end()
